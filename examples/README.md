...
| `multi_background_case.json` | **Authored here as an illustration**, modelled on a hand-written radiotherapy follow-up session: several timepoints as switchable backgrounds over a shared `data[]` pool, `options.plugin` naming the server-side reader, `imageSmoothingEnabled: false` on the integer label volumes, `order` naming shaders by key, and shaders carrying more than one `dataReferences` entry. Not a copy of anyone's file — replace it with the real one when you have it. It also carries a param nobody intended: `inverse` on its three `rtstruct` **colormap** layers. `heatmap` declares `inverse` and `colormap` does not, so the identically-spelled param on the sibling `dose` layers is fine and these three are dropped by v3 in silence — the audit reports them as notes, not errors, and `tests/test_audit.py::test_the_multi_background_case_is_expected_to_carry_one_stale_param` pins all six outcomes. Kept on purpose: invented keys arrive in other people's files, the viewer discards them without a word, and the only thing standing between a colleague and a silently missing layer is a tool that says so. |
